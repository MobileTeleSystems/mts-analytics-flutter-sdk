...
```yaml
  mts_analytics_plugin:
    git:
      url: git@gitlab.services.mts.ru:mts-metric/mobile/flutter/flutter-sdk.git
      ref: 1.6.0
```

Пример подключения: <https://gitlab.services.mts.ru/mts-metric/mobile/flutter/flutter-sdk/-/tree/master/example``>
...
allprojects {
    repositories {
        maven {
            ...
            url "https://artifactory.mts.ru/artifactory/maven-public-mtsm-maven-local/"
        }

        maven {
            name "nexus"
            url 'https://nexus-cache.services.mts.ru/repository/maven-public/'
        }
    }
}
```
...
```ruby
source 'https://gitlab.services.mts.ru/mts-analytics/ios/podspecs'
